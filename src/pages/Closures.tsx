...
export default function Closures() {
  ...
  // Stats for the "Auto-Period" (Current Month)
      ...
      <div className="animate-fade-in space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-[13px] mt-1">Audit trail e encerramento de períodos — {organization.name}</p>
          </div>
        </div>

        {/* Current period auto-card */}
        <Card className="bg-card border-border border-l-4 border-l-primary/40">
          ...
          <CardContent className="pb-4 pt-2">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div><p className="text-[10px] text-muted-foreground uppercase font-semibold">Movimentações</p><p className="text-lg font-bold font-mono">{currentPeriodStats.count}</p></div>
              <div><p className="text-[10px] text-muted-foreground uppercase font-semibold">Entradas</p><p className="text-lg font-bold font-mono text-success">{formatCurrency(currentPeriodStats.income)}</p></div>
              <div><p className="text-[10px] text-muted-foreground uppercase font-semibold">Saídas</p><p className="text-lg font-bold font-mono text-destructive">{formatCurrency(currentPeriodStats.expense)}</p></div>
        ...
        <div className="pt-4">
          <h2 className="text-[12px] font-bold text-muted-foreground uppercase tracking-widest mb-4">Histórico de Fechamentos</h2>
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {!expandedTxs[closure.id] ? (
                                <TableRow><TableCell colSpan={4} className="text-center py-4"><Loader2 className="h-4 w-4 animate-spin mx-auto text-primary" /></TableCell></TableRow>
