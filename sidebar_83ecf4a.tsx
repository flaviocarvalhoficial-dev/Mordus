...
const tesourariaItems = [
  ...
  { title: "Lan├ºamentos", url: "/lancamentos", icon: ArrowUpDown },
...
const secretariaPessoas = [
  ...
  { title: "Lideran├ºa", url: "/lideranca", icon: Crown },
  { title: "Departamentos", url: "/departamentos", icon: UsersRound },
];

const secretariaAdm = [
  { title: "Patrim├┤nio", url: "/patrimonio", icon: Building2 },
  { title: "Documentos", url: "/documentos", icon: File },
  { title: "Congrega├º├Áes", url: "/congregacoes", icon: Home },
];

const secretariaSocial = [
  { title: "Calend├írio", url: "/calendario", icon: CalendarDays },
  { title: "Servi├ºo Social", url: "/servico-social", icon: Heart },
  { title: "Parceiros", url: "/parceiros", icon: Handshake },
];

const configItems = [
  { title: "Configura├º├Áes", url: "/configuracoes", icon: Settings },
];
  ...
  return (
    <Sidebar collapsible="icon">
      ...
      <SidebarContent>
        ...
        {renderGroup("Membros & Lideran├ºa", secretariaPessoas)}
        {renderGroup("Administrativo", secretariaAdm)}
        {renderGroup("Comunit├írio & Social", secretariaSocial)}
      ...
      <SidebarFooter className="p-3 border-t border-sidebar-border space-y-2">
        ...
        {!collapsed && activeSocials.length === 0 && (
          <p className="text-[11px] text-muted-foreground text-center">v1.0 ÔÇö MVP</p>
        )}
      </SidebarFooter>
    </Sidebar>
  );
